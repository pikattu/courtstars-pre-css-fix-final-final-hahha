...
function findPythonWithNbaApi(): ?string {
    // Use file_exists() to skip missing binaries without shell overhead.
    // Check print('ok') so we confirm nba_api actually imports successfully.
    $candidates = [
    ...
    foreach ($candidates as $bin) {
        ...
        $out = shell_exec("$bin -c \"import nba_api\" 2>&1");
        if (trim((string)$out) === '') {
            return $bin;
        }
    }
...
function syncStats(PDO $pdo): array {
    ...
    // We match on api_player_id (nba_id from Python)
